        ...
        elseif($_GET['action']=='status'){
            ...
            if(!empty($_POST['submit'])){ //jika GET_act maka masok 
                if($_POST['submit'] == 'Simpan'){ //jika submit bernilai simpan maka masok
                    ...
                    if ($SQL) { //jika simpan berhasil maka menampilkan alert dan kembali 
                        echo" <script language='JavaScript'> 
                            alert('Data Status Berhasil diinput!'); 
                            window.location='../home.php?page=input-status';
                            </script>";
                    }
                    else{
                        echo" <script language='JavaScript'> 
                            alert('Data Status Gagal diinput!'); 
                            window.location=history.go(-1) 
                            </script>";
                    }
            ...
            //DELETE 
            elseif(!empty($_GET['del'] OR $_GET['del']==0)){ //jika ada GET bernilai del atau del berisi nol maka masok proses hapus
                mysql_query("DELETE FROM tb_status WHERE id_status=".$_GET['del']); //pokoke proses delete
                echo" <script language='JavaScript'>
                            alert('Data Status Berhasil dihapus!');
                            window.location='../home.php?page=input-status';
                            </script>"; // balik maning
            }
            else{ 
        ...
        elseif($_GET['action']=='today'){
            ...
            if(!empty($_POST['submit'])){ //jika GET_act maka masok 
                if($_POST['submit'] == 'Ubah'){ //jika submit bernilai ubah maka masok
                    ...
                    if($_POST['status']=='2'){
                        mysql_query("
                        INSERT INTO tb_presensi (id_matakuliah, dimulai, id_ruangan, id_status)
                            VALUES ('$_POST[id_matakuliah]', '$tgl','$_POST[ruangan]','$_POST[status]')
                        ");
                    }
            ...
            elseif(!empty($_GET['del'] OR $_GET['del']!=0)){ //jika ada GET bernilai del atau del berisi nol maka masok proses hapus
                ...
                mysql_query("
                    ...
                    WHERE
                        ...
                        dimulai ='".$data['dimulai']."'
                    ");
                mysql_query("DELETE FROM tb_today WHERE id_today=".$_GET['del']);
                echo "<script language='JavaScript'>
                            alert('Data Jadwalkuliah Berhasil dihapus!');
                            window.location=history.go(-1);
                            </script>"; 
